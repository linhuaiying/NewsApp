...
public class EntertainmentNewsPresenter <T extends INewsView> extends BasePresenter {
    //2.持有Model接口
    NewsModel entertainmentNewsModel = new EntertainmentNewsModel();

    @Override
    //3.执行业务逻辑
    public void fetch() throws InterruptedException {
        if(iView.get() != null && entertainmentNewsModel != null) {
            entertainmentNewsModel.loadNewsData(new NewsModel.OnLoadListener() {
                @Override
