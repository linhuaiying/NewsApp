package com.example.newsapp.Model.MyFavNewsModels;
...
public class MyFavNewsModel {
    String username = "";
    public MyFavNewsModel(String username) {
        this.username = username;
    }
    public void loadFavNewsData(MyFavNewsModel.OnLoadListener onLoadListener) throws InterruptedException {
        onLoadListener.onComplete(getData());
    }
    public interface OnLoadListener{ //判断数据是否成功接收
        void onComplete(List<UsersNews> myFavNewsList); //成功了就拿到数据
        void onError(String msg);
    }
    //该方法想象成数据是从网络或者数据库或者服务、、、地方来的
    public List<UsersNews> getData() throws InterruptedException {
        List<UsersNews>[] myFavNewsList = new List[]{new ArrayList<>()};
        ...
        Call<List<UsersNews>> call = usersNewsService.getFavNewsList(username);
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                try {
                    Response<List<UsersNews>> response = call.execute();
                    myFavNewsList[0] = response.body();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
